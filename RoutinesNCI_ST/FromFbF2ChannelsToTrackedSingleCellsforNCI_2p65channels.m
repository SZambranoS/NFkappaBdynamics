function [] = FromFbF2ChannelsToTrackedSingleCellsforNCI_2p65channels(pathinput,filenamepoints,filenamefinal,pathoutput,vparamstracking)
...
for k=1:nframes
    ...
    BG{k}=OUTPUT{k}.BG';
    REDNUCLEAR{k}=OUTPUT{k}.REDNUCLEAR';
    REDRING{k}=OUTPUT{k}.REDRING'; 
    AverageBGQUANT(k)=OUTPUT{k}.BG';
    AverageBGQUANTRED(k)=OUTPUT{k}.REDBG'
...
all_REDNUCLEAR=vertcat(REDNUCLEAR{:});
all_REDRING=vertcat(REDRING{:});
...
matrixREDNUCLEAR=NaN(length(points),n_tracks);
matrixREDRING=NaN(length(points),n_tracks);
...
for i_track = 1 : n_tracks
    ...
    matrixREDNUCLEAR(track_frames,i_track)=all_REDNUCLEAR(track,:);
    matrixREDRING(track_frames,i_track)=all_REDRING(track,:);
end
...
matrixREDNUCLEAR=InterpolateMatrixofNaNs(matrixREDNUCLEAR);
matrixREDRING=InterpolateMatrixofNaNs(matrixREDRING);



cd(pathoutput)
save(filenamefinal,'matricesofXtracks','matricesofYtracks','matrixQUANT','matrixareas','matrixTRACK','matrixINTRING','matrixAREARING','matrixREDNUCLEAR','matrixREDRING','AverageBGQUANT','AverageBGQUANTRED'); 
