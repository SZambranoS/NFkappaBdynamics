function plot_NCIResults_boundaries(pathInput,Results, Stack)
...
hFrameSlider = uicontrol( ...
    ...
    'Callback', @(src, evt) changeFrame(hStack, hPlot, get(src, 'Value'), ...
     OUT, Stack, nFrames,limitstouse,maxaxis,matrixNCI,Bcyto,Bnuc));
...
Bcytothisframe=Bcyto{iFrame};
Bnucthisframe=Bnuc{iFrame};
...
hold on;
 for k = 1:length(Bnucthisframe)
            disp('Cucu')
            length(Bnucthisframe)
            boundary = Bnucthisframe{k};
            plot(hStack,boundary(:,2), boundary(:,1), 'k', 'LineWidth', 2);
 end
hold off
