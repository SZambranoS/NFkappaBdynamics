function [Objects,roughmask,refinedmask] = segment_fromImage_refined_cluster_FINER(img, par)
...
if sigmarough>0
    FilterImg = imfilter(img,fspecial('gaussian', 20, sigmarough),'same');
else 
    FilterImg=img;
...
% Get the objects thresholded 
   ...
   % ObjectsProv=segment_fromMask(ThrImg, [0.25*minSize,1.5*maxSize]);    %Notice that for the provisory objects we are more loose about the cells considered
   ObjectsProv=segment_fromMask(ThrImg, [round(0.25*minSize),10*maxSize]);    %Notice that for the provisory objects we are more loose about the cells considered
