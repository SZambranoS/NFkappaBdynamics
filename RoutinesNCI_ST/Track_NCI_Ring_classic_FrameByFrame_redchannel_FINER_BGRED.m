function [LabelsMap, cellROIs_refined,refinedmask,roughmask] = Track_NCI_Ring_classic_FrameByFrame_redchannel_FINER_BGRED(StackTrack,StackQuant, StackRed, parCellTrack, filenameprint, filenamesave, shallweplot,pathoutput)
...
%February 2020. This allows for a finer control of the segmentation, useful for crowded
%fields 

%July 2020: Red channel background is also analyzed

parCellSeg=parCellTrack(1:6);
TolArea=parCellTrack(7);
RingWidth=parCellTrack(8);
FactorBG=parCellTrack(9);
sigmaforring=parCellTrack(10);
...
for i = 1:nFrames;
    [cellROIs_refined(i),roughmask(i).data,refinedmask(i).data] = segment_fromImage_refined_cluster_FINER(StackTrack(i).data, parCellSeg);
    disp(i)
end
clear i
...
[LabelsMap, LabelsRing, OUT, nCells,TotalintensityTrack,TotalintensityQuant,TotalintensitySignalQuant,ObjectsPerFrame,AverageBGQUANT,AverageBGRed] = track_and_quantify_2channels_forNuctoCyto_redchannel_bgred(cellROIs_refined(i), StackTrack(i), StackQuant(i), StackRed(i), 1, TolArea,RingWidth,FactorBG,sigmaforring);
...
RedRing=-1*ones(1,nCells);
RedNuclear=-1*ones(1,nCells);
nCells
...
for n=1:nCells
    ...
    RedNuclear(1,n)=OUT{n}.RedNuclear;
    RedRing(1,n)=OUT{n}.RedRing;
...
OUTPUT{i}.REDBG=AverageBGRed;
OUTPUT{i}.REDNUCLEAR=RedNuclear;
OUTPUT{i}.REDRING=RedRing;
