function [LabelsMap, LabelsRing, OUT, nCells,TotalintensityTrack,TotalintensityQuant, TotalintensityQuantSignal, ObjectsPerFrame,AverageBGQuant,AverageBGRed] = track_and_quantify_2channels_forNuctoCyto_redchannel_bgred(cellROIs, StackTrack, StackQuant, StackRed, nFrames, TolArea,WidthRing,FactorBG,sigmafilter)
...
%(SZ, April 2018) We corrected the way in which we . Importantly, one has
%to specify the sigma used for the smoothing to calculate the ring around
%the cytosol. However, both nuclear and cytosolic amount are to be used
%with the original (unfiltered) image. 

%(SZ, June 2018): It now fills the holes of the cytoplasms. 

%(SZ, July 2019) This new program allows one to quantify also a second red
%signal; 
...
for iFrame = 1:nFrames
    ...
    p65signalimage=(FilterImgQuant(iFrame).data)>threshold(iFrame);
    
    BGSignalimage=valueminmean*p65signalimage; 
    ...
    meantilesred = blockproc(StackRed(iFrame).data,[32 32],fmeantiles);
    [valueminmeanred,idminmeanred]=min(meantilesred(:));
    AverageBGRed(iFrame)=valueminmeanred; 
...
%     colorbar
%     figure(900)
%     imagesc(p65signalimage);
%     colorbar
%     figure(899)
%     imagesc(BGSignalimage);
%     colorbar
%     
    TotalintensityQuantSignal(iFrame)=sum(sum(p65signalimage.*(double(StackQuant(iFrame).data))))-sum(sum(BGSignalimage)); 
    ...
    for iFrame = 1:nFrames; %cycle on frame
        ...
        if ~isnan(Baricenter{iFrame}(iCell,1))      % if the cell exist at iFrame
            ...
            OUT{iCell}.TotalIntensityQuant(iFrame,1) =...                           % Compute total Intensity of Stack for Quantification
             ...
             OUT{iCell}.RedNuclear(iFrame,1) =...                           % Compute total Intensity of Stack for Tracking
                sum(sum(double(StackRed(iFrame).data).*(MaskiCell)));
          ...
          dilatedMask = imdilate(MaskiCell,se);
           ...
           matrixRingvalues=MatrixRing.*MaskRing;
            vRing=matrixRingvalues(:);
           %vRing=vRing(find(vRing));
           vRing=vRing((vRing>0));
           
           threshold=mean(vRing);   %%%%%%%%%
           MaskRing=(MatrixRing>threshold);
           ...
           %To avoid cytoplasms superpositions. From here
           
           MaskRingSupp=MaskRing; 
           
           MaskRing(LabelsRing(iFrame).data>0)=0; 
           LabelsRing(iFrame).data(MaskRingSupp)=0; 
...
LabelsRing(iFrame).data(MaskRing)=iCell; 
...
%            pause(0.5);
           ...
           MatrixRed=MaskRing.*(double(StackRed(iFrame).data));
         ...
         OUT{iCell}.RedRing(iFrame,1) =sum(sum(double(StackRed(iFrame).data).*(MaskRing)));
