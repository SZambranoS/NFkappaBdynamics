function [LabelsMap, cellROIs_refined,refinedmask,roughmask] = Track_NCI_Ring_classic_FrameByFrame_redchannel_FINER_shorter(StackTrack,StackQuant, StackRed, parCellTrack, filenameprint, filenamesave, shallweplot,pathoutput,nmax)
...
[LabelsMap, LabelsRing, OUT, nCells,TotalintensityTrack,TotalintensityQuant,TotalintensitySignalQuant,ObjectsPerFrame,AverageBGQUANT] = track_and_quantify_2channels_forNuctoCyto_classical_redchannel(cellROIs_refined(i), StackTrack(i), StackQuant(i), StackRed(i), 1, TolArea,RingWidth,FactorBG,sigmaforring);
...
OUTPUT{i}.BoundRing=Bring;
    ...
    if (strcmp(shallweplot,'QUANT')||(strcmp(shallweplot,'BOTH'))||(strcmp(shallweplot,'BOTHANDRINGS'))||(strcmp(shallweplot,'QUANTANDRINGS')))
        ...
        imagesc(FilterImg, [min(FilterImg(:)),4*mean(FilterImg(:))]);
        colormap('hot');
        %colormap('jet');
        hold on;
